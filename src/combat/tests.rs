...
#[test]
fn chaser_moves_toward_player_in_three_dimensions_at_time_based_speed() {
    for dt in [1_f32 / 30., 1. / 120.] {
        ...
        let expected = from + (START - from).normalize() * 75.;
        assert!(position(&app, chaser).distance(expected) < 0.01);
    }
}
...
fn death_clamps_health_and_freezes_all_gameplay_until_restart() {
    ...
    app.world_mut().resource_mut::<CombatConfig>().chase_speed = 150.;
