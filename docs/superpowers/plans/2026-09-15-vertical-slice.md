...
- [x] Run `cargo test --locked` on the unchanged baseline.
- [x] Add `src/combat/slice_tests.rs`, registered as a test module in `src/combat.rs`.
...
- [x] Complete mission 01 without granting funds/health/XP or disabling waves; log
  elapsed time, kills, hull, loot and naturally earned upgrades.
- [x] Use actual shop keyboard actions to buy/equip contrasting affordable modules,
  recreate the app from its saved file, and fly mission 02 through all sites and exit.
- [x] Record failures as well as successful strategies. Identify the largest concrete
  pacing/progression issue before changing production behavior.

## 2. Fix and verify the measured issue

- [x] Add a failing regression test for the chosen issue.
- [x] Implement the smallest fix in the relevant mission/shop UI or rules module.
- [x] Verify the regression, preservation of replay/retry/selection and save behavior,
  and repeat both campaign probes. Commit the verified behavior change.

## 3. Native presentation, evidence and delivery

- [x] Exercise the changed loop presentation with `cargo dev` at 1120x720 and 640x480.
